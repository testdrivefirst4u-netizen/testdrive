...
export const metadata: Metadata = buildMetadata({
  title: "Compare Bikes & Scooters Side by Side - Specs, Price & Features",
  description: "Compare motorcycles and scooters side by side. Check engine, mileage, features, and price to find your perfect two-wheeler.",
  keywords: "compare bikes india, bike vs bike comparison, scooter comparison, best bike 2025",
  canonicalPath: "/bikes/compare",
});
...
function PairCard({ a, b }: { a: { name: string; slug: string; priceDisplay: string | null; priceMin: number | null; images: Array<{ url: string }> }; b: typeof a }) {
  ...
  return (
    <Link href={`/bikes/compare/${a.slug}-vs-${b.slug}`}
...
export default async function BikesComparePage() {
      ...
      <div className="bg-gradient-to-r from-slate-900 via-blue-950 to-blue-900 text-white py-10">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
          ...
          <div className="flex items-center gap-2.5 mb-2">
            ...
            <h1 className="text-2xl sm:text-3xl font-bold">Compare Bikes &amp; Scooters</h1>
          </div>
          <p className="text-blue-200 text-sm max-w-xl">Select brand, pick models and compare specs, mileage, and features side by side.</p>
        </div>
      </div>

      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-8 space-y-10">
        <div>
          <h2 className="text-lg font-bold text-slate-900 mb-4">Choose Two-Wheelers to Compare</h2>
          <BrandVehicleSelector compareBasePath="/bikes/compare" types={BIKE_TYPES} maxSelect={4} />
        </div>
        {pairs.length >= 2 && (
          <div>
            <div className="flex items-center gap-2 mb-5">
              <TrendingUp className="w-4 h-4 text-blue-600" />
              <h2 className="text-lg font-bold text-slate-900">Popular Bike Comparisons</h2>
            </div>
