    ...
    <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-6 space-y-8">
      ...
      <div className="grid lg:grid-cols-[1fr_400px] gap-8">
        <div className="space-y-6">
          ...
          {/* EV-specific: Range + charging info */}
          <div className="grid grid-cols-3 gap-3">
            {["Range","Charging","Battery"].map(label => (
              <div key={label} className="bg-white rounded-2xl border border-gray-100 p-4 text-center space-y-2">
                <S c="h-8 w-20 mx-auto" />
                <S c="h-3.5 w-16 mx-auto" />
              </div>
            ))}
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-4 space-y-3">
            ...
            <div className="flex gap-2 flex-wrap">
              {Array.from({ length: 3 }).map((_, i) => <S key={i} c="h-9 w-32 rounded-xl" />)}
            </div>
          </div>
